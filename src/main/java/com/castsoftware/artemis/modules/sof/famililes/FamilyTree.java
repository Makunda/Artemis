...
package com.castsoftware.artemis.modules.sof.famililes;
...
public class FamilyTree {
  ...
  /**
   * Explode the name in multiple leafs. If the leafs are too small , they will be merged back in
  ...
  public int increaseDepth() {
    ...
    while (!toVisit.isEmpty()) {
      ...
      if (!actualFl.children.isEmpty()) {
        ...
        log.info(
            String.format(
                "KeySet for level for level %d : %s",
                actualFl.depth,
                actualFl.items.stream()
                    .map(x -> (String) x.getProperty("Name"))
                    .collect(Collectors.joining(" ,"))));
        Map<String, List<Node>> newFamilies =
            explodeName(depth, actualFl.items); // Explode the names

        // Create new leaves
        for (Map.Entry<String, List<Node>> en : newFamilies.entrySet()) {
          FamilyLeaf familyLeaf =
              new FamilyLeaf(en.getKey(), depth, new ArrayList<>(), en.getValue());
  ...
  public void displayFinalTree(FamilyLeaf fl) {
    String mes =
        String.format(
            "%s : Prefix %s : Size : %s", "-".repeat(fl.depth * 2), fl.name, fl.children.size());
    if (fl.breakpoint) {
      mes += " -- IS BREAKPOINT --";
    }
    log.info(mes);
  ...
  /**
   * Check if leaf is a breakPoint. A break point is a leaf having a better distribution than its
   * children
   *
  ...
  public List<FamilyLeaf> getEndLeaves() {
    ...
    while (!toVisit.isEmpty()) {
      ...
      if (fl.children.isEmpty() || (!breakpointUnderBranch(fl) && fl.depth > MIN_PREFIX)) {
        ...
        for (FamilyLeaf child : fl.children) toVisit.push(child);
      }
    }
