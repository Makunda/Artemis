  ...
  public static List<FrameworkNode> getUserCustomFrameworks(Neo4jAL neo4jAL)
    ...
    if (UserConfiguration.isKey(neo4jAL, "artemis.tag.framework.identifier")) {
      ...
      if (!alreadyWarned)
        neo4jAL.logInfo(
            String.format(
                "No artemis.tag.framework.identifier found in the user configuration. Will use the default : '%s' tag",
                artemisFrameworkTag));
      alreadyWarned = true;
    }

    String req =
        "MATCH (o:Object) WHERE single( x in o.Tags WHERE x CONTAINS $tagArtemis) "
    ...
    while (res.hasNext()) {
      ...
      try {
        ...
        neo4jAL.logError(
            String.format("Failed to create framework for object with name : %s", name));
      }
    }
  ...
  public static boolean isTagPresent(Neo4jAL neo4jAL) throws Neo4jQueryException {
    ...
    if (UserConfiguration.isKey(neo4jAL, "artemis.tag.framework.identifier")) {
      ...
      if (!alreadyWarned)
        neo4jAL.logInfo(
            String.format(
                "No artemis.tag.framework.identifier parameter found in the user configuration. Will use the default one '%s'.",
                artemisFrameworkTag));
      alreadyWarned = true;
    }
