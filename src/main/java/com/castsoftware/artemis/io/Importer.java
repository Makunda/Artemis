...
public class Importer {
    ...
    private void createNode(Label label, List<String> headers, List<String> values) throws Neo4jQueryException {
        ...
        try {
            ...
            if(!nameId.isEmpty()) {
                String req = String.format("MATCH (o:%s) WHERE o.Name='%s' return o", FRAMEWORK_LABEL, nameId);
    ...
    public Stream<String> load(String pathToZipFileName) throws ProcedureException {
        ...
        try  {
            ...
            if (!zipFile.exists()) {
                MESSAGE_QUEUE.add("No zip file found at path ".concat(pathToZipFileName).concat(". Please check the path provided"));
        ...
        MESSAGE_QUEUE.add(String.format("%d file(s) containing a label where found and processed.", countLabelCreated));
        MESSAGE_QUEUE.add(String.format("%d file(s) containing relationships where found and processed.", countRelationTypeCreated));
        MESSAGE_QUEUE.add(String.format("%d file(s) where ignored. Check logs for more information.", ignoredFile));
        MESSAGE_QUEUE.add(String.format("%d node(s) and %d relationship(s) were created during the import.", nodeCreated, relationshipCreated));
